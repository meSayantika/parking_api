...
const vehicle_wise = async (req, res) => {
    try {
        ...
        var select = `d.vehicle_name vehicleType, COUNT(b.receipt_no) tot_vehi, SUM(c.paid_amt) tot_amt, SUM(c.advance_amt) advance_amt`,
            table_name = 'v_vehicle_in a, v_vehicle_out b, v_receipt c, md_vehicle d',
...
const unbilled = async (req, res) => {
    try {
        ...
        var select = `a.receipt_no, a.date_time_in, a.device_id, d.vehicle_name, a.vehicle_no, f.operator_name, g.advance_amt`,
            table_name = `v_vehicle_in a JOIN  md_vehicle d ON  a.vehicle_id=d.vehicle_id
            JOIN md_user e  ON a.user_id_in=e.id 
            JOIN md_operator f ON e.user_id=f.user_id  
            LEFT JOIN v_receipt g ON a.receipt_no = g.receipt_no`,
...
const detail_report = async (req, res) => {
    try {
        ...
        var select = `a.receipt_no, a.date_time_in, a.device_id, d.vehicle_name, a.vehicle_no, b.date_time_out, b.device_id device_id_out, c.base_amt, c.cgst, c.sgst, c.paid_amt, c.advance_amt, c.pay_mode, f.operator_name`,
            table_name = 'v_vehicle_in a, v_vehicle_out b, v_receipt c, md_vehicle d, md_user e, md_operator f',
