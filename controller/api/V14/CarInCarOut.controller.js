...
const { vehicle_in, insert_receipt, insert_vehicle_outpass, update_car_in_flag, insert_advance_receipt_update, outpass_advance_receipt_update } = require("../../../module/V12/car_in_out_receipt");
    ...
    try {
        ...
        if ((md_setting.msg).length != 0) {
            ...
            if (md_setting_data.dev_mod == 'D' || md_setting_data.dev_mod == 'R') {
                ...
                console.log(td_vehicle_in.receipt_number, "***********");
                if (md_setting_data.adv_pay == 'Y') {
                    let receipt = await insert_advance_receipt_update(userData, td_vehicle_in.receipt_number, 0, value.base_amt, value.cgst, value.sgst, value.igst, value.paid_amt, value.gst_flag, 'A')
                }
...
const search_car = async (req, res) => {
    ...
    if (dev_mod == 'D' || dev_mod == 'B') {
        ...
        let search_car = await db_Select('a.*,b.vehicle_name', 'td_vehicle_in a, md_vehicle b', where, `ORDER BY a.date_time_in DESC`)
...
const search_car_scan = async (req, res) => {
    ...
    if (dev_mod == 'D' || dev_mod == 'B') {
        ...
        let search_car_scan = await db_Select('a.*,b.vehicle_name', 'td_vehicle_in a, md_vehicle b', where, `ORDER BY a.date_time_in DESC`)
...
const out_pass = async (req, res) => {
    ...
    if (search_car.msg.length == 0) {
        ...
        if (vehicle_outpass.suc == 1) {
            if (adv_pay == 'Y') {
                var receipt = await outpass_advance_receipt_update(userData, value.receipt_no, value.base_amt, value.cgst, value.sgst, value.igst, value.paid_amt, value.gst_flag, 'A', value.paymode)
            } else {
                var receipt = await insert_receipt(userData, value.receipt_no, value.base_amt, value.cgst, value.sgst, value.igst, value.paid_amt, value.gst_flag, 'P', value.paymode)
...
const manual_car_in_out = async (req, res) => {
    try {
        const schema = Joi.object({
            vehicle_id: Joi.required(),
            vehicle_no: Joi.string().required(),
        ...
        if (md_setting && md_setting.msg && md_setting.msg.length > 0) {
            dev_mod = md_setting.msg[0].dev_mod;
        ...
        if (td_vehicle_in.suc == 1) {
            ...
            if (receipt.suc == 1) {
                ...
                res.json(sendErrorResponce(null, { message: 'Failed to insert receipt' }));
            }
        } else {
            res.json(sendErrorResponce(null, { message: 'Failed to insert vehicle in' }));
        }
...
module.exports = { car_in, search_car, out_pass, car_in_fixed, car_advance_amount, car_in_out, search_car_scan, manual_car_in_out };