...
const my_profile_save = async (req, res) =>{
  try {
    ...
    let fields = `location_id = ${data.loc_id},mobile_no = ${data.phone},email = '${data.email_id}',cust_addr = '${data.cust_add}',upi_id = '${data.upi_id ? data.upi_id : ''}',updated_at='${datetime}'`,
      where = `customer_id='${custId}'`;
    ...
    if(res_dt.suc > 0){
      ...
      req.session.user.user_data.upi_id = data.upi_id
    }
    // console.log(res_dt);
