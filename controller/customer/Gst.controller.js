...
const gst_dtls = async (req, res) => {
  ...
  const add_gst = async (req, res) => {
    try {
      ...
      var custId = req.session.user.user_data.customer_id;
        ...
        const fields = "(customer_id,gst_flag,gst_type,gst_mode,gst_number,total_gst,cgst,sgst,igst,created_by,created_at)",
        values = `('${custId}','Y','I','${value.gst_type}','${value.gst_num}','${value.total_gst}',${value.gst_type == 'CS' ? `'${value.cgst ? value.cgst.replace('%', '') : 0.00}'`: `'0.00'`},${value.gst_type == 'CS' ? `'${value.sgst ? value.sgst.replace('%', '') : 0.00}'`: `'0.00'`},${value.gst_type == 'I' ? `'${value.igst ? value.igst.replace('%', '') : 0.00}'`: `'0.00'`},'${custId}','${datetime}')`
        whr = null,
        flag = 0;
  ...
  const edit_save_gst = async (req, res) => {
    try {
      ...
      let fields =`gst_mode = '${value.gst_type}',gst_number='${value.gst_num}',total_gst='${value.total_gst}',cgst=${value.gst_type == 'CS' ? `'${value.cgst ? value.cgst.replace('%', '') : 0.00}'`: `'0.00'`},sgst=${value.gst_type == 'CS' ? `'${value.sgst ? value.sgst.replace('%', '') : 0.00}'`: `'0.00'`},igst=${value.gst_type == 'I' ? `'${value.igst ? value.igst.replace('%', '') : 0.00}'`: `'0.00'`},updated_by = '${custId}',updated_at='${datetime}'`,
