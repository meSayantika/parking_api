  ...
  const save_add_operator = async (req, res) => {
    try {
      ...
      var password = bcrypt.hashSync(value.pwd.toString(), 10);
        ...
        let mobileCondition =
      value.id > 0
        ? `user_id = '${value.mobile}' AND user_id NOT IN (SELECT user_id FROM md_operator WHERE operator_id = '${value.id}' AND customer_id = '${value.cust_id}')`
        : `user_id = '${value.mobile}'`;

    const user = await db_Select("user_id", "md_user", mobileCondition, null);
