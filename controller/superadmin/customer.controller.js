...
const save_add_customer = async (req, res) => {
  try {
    ...
    let fields =
        value.id > 0
          ? `seller_id='${value.sell_name}',customer_name='${value.cust_name}',location_id='${value.loc_name}',mobile_no='${value.phone}',email='${value.email}',cust_addr='${value.cust_add}',dev_mod='${value.dev_mode}',no_device='${value.no_device}',customer_type='${value.cust_type}',upi_id = '${value.upi_id ? value.upi_id : ''}',updated_by='${user_name}',updated_at='${datetime}'`
          : "(seller_id,customer_name,location_id,mobile_no,email,cust_addr,dev_mod,no_device,customer_type,device_type,upi_id,created_by,created_at)",
      values = `('${value.sell_name}','${value.cust_name}','${value.loc_name}','${value.phone}','${value.email}','${value.cust_add}','${value.dev_mode}','${value.no_device}','${value.cust_type}','H','${value.upi_id ? value.upi_id : ''}','${user_name}','${datetime}')`;
